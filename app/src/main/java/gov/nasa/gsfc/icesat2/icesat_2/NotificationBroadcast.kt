...
class NotificationBroadcast : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        Log.d(TAG, "onReceive - NotificationBroadcast")
        val nm = NotificationsSharedPref(context!!)
        /**
         * When the device restarts all the notifications are lost. This method adds all the notifications
         * back when it detects that the device has powered back on. It retrieves the information about
        ...
        if (intent?.action == "android.intent.action.BOOT_COMPLETED") {
            ...
            notificationKeys.forEach {
                ...
                if (timeOfAlarm < System.currentTimeMillis()) {
                    ...
                    val pendingIntent = PendingIntent.getBroadcast(context, it.hashCode(), myIntent,
        ...
        Log.d(TAG, "notificationBroadcast flyoverTimeKey is $flyoverTimeKey")
    ...
    companion object {
        ...
        fun createNotification(context: Context, lat: Double?, long: Double?, timeString: String?, time: Long, paramSearchString: String?, dateString: String?, hours:String?) {
            Log.d(TAG, "createNotification - NotificationBroadcast")
