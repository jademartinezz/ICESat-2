...
GoogleMap.OnPolylineClickListener, IShareAndCalendar {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mainActivityViewModel?.pastPointsList?.value?.let { pointList.addAll(it) }
        mainActivityViewModel?.allPointsList?.value?.let { pointList.addAll(it) }
        ...
        checkBoxMarker.setOnClickListener {
            markerList.forEach { it.isVisible = checkBoxMarker.isChecked }
        }

        checkBoxPath.setOnClickListener {
            polylineList.forEach { it.isVisible = checkBoxPath.isChecked }
        }
        ...
        checkBoxLasers.setOnClickListener {
            ...
            laserPolyLines.forEach { it.isVisible = checkBoxLasers.isChecked }
        }
    }

    override fun onViewStateRestored(savedInstanceState: Bundle?) {
        Log.d(TAG, "onViewStateRestored - MapFragment")
    ...
    private fun addChainPolyline(chain: ArrayList<Point>) {
        Log.d(TAG, "addChainPolyline - MapFragment")
        val polylineTag = 0
        val polylineOptions = PolylineOptions()
        val pastColor = 0xff007fff.toInt()

        for (i in 0 until chain.size) {
            val latitude = chain[i].latitude // Default value if null
            val longitude = chain[i].longitude // Default value if null

            val markerAdded = mMap.addMarker(
                MarkerOptions()
                    .position(LatLng(latitude, longitude))
                    .title(getString(R.string.latLngDisplayString, latitude.toString(), 0x00B0.toChar(), longitude.toString(), 0x00B0.toChar()))
            )
...
//        onMapClick(p0 = null)
    ...
    private fun calculateLaserBeams() {
        Log.d(TAG, "calculateLaserBeams - MapFragment")
